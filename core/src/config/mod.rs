use crate::{
    ...
    construct::{self, space::WorkPoint, Component, Error},
};
use regex::Regex;
...
pub struct Config {
    pub size: DataHV<f32>,
    pub expand_size: DataHV<f32>,
    pub white_area_size: DataHV<f32>,
    ...
    pub surround_align: DataHV<BTreeMap<Place, f32>>,

    pub reduce_trigger: DataHV<f32>,
    pub center_weights: BTreeMap<LineType, f32>,
}
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            expand_size: DataHV::splat(0.0),
            white_area_size: DataHV::splat(0.0),
            ...
            surround_align: DataHV::splat(BTreeMap::from([(Place::Start, 0.0), (Place::End, 0.0)])),
            reduce_trigger: DataHV::splat(0.0),
            center_weights: Default::default(),
        }
    }
}
...
impl Config {
    ...
    pub fn get_white_area_weight(&self, elements: &Vec<Vec<Element>>, axis: Axis) -> f32 {
        ...
        if elements.is_empty() || elements.iter().all(|els| els.is_empty()) {
            ...
            let face_weight = {
                let count = elements
                    .iter()
                    .filter(|els| els.contains(&Element::Face))
                    .count();
                match count {
                    0 => 1.0,
                    _ => (1.0 - count as f32 / elements.len() as f32) * s_weight,
                }
            };
            let other_weight = elements
                .iter()
    ...
    pub fn expand_comb_proto(
        ...
        let min_len = min_vals
            ...
            .unwrap();
        let center = comb
            .visual_center(min_len, true, &self.center_weights)
            .to_hv_data();
                            ...
                            *i_allowances = vec![0.0; intervals.len()];
                            if surround == Place::Mind {
                                ...
                                if s_edge_key {
                                    ...
                                    let surround_align = self.surround_align.hv_get(axis)
                                        [&surround.inverse()]
                                        .clamp(-1.0, 1.0);

                                    base_len = if s_base_len > 0 {
                                        let max_len =
                                            subarea_assign - i_bases.iter().sum::<f32>() - min_val;

                                        let offset_corr = if surround == Place::End {
                                            corr_vals[0]
                                        } else {
                                            corr_vals[1]
                                        };
                                    ...
                                    let s_offset_val =
                                        subarea_assign - i_bases[0] - i_allowances[0] - base_len;
                                    if surround == Place::End {
                                        s_offset[0] = s_offset_val;
                                    } else {
                                        s_offset[1] = s_offset_val;
                                    }
                                }
                            }
